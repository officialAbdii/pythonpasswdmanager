...
import psycopg2
import argparse
# This module is used to create a CLI for the user to add, delete, update, or query entries.
...
def main():
    ...
    master_password_hash = hashlib.sha256(master_password_input).hexdigest()
    # The user is prompted to enter their master password which is then hashed and compared.

    if master_password.query_master_pwd(master_password_input) is True:
        
        connection = db_connect.connection_db()
        # Establishes a connection to the database using db_connect.connection_db(). The 
    ...
    # cursor object is used to execute SQL queries on the database.

    connection.commit()

    # The if statements in the script check which command-line argument was provided and execute the 
    # corresponding SQL query to add, delete, update, or query for a password entry. 
    # If the --list argument is provided, the script executes a SQL query to list all entries.
    ...
    if args.add:
        ...
        cursor.execute(sql_statements.insert_db_row(), (URL, username, password_official))
    ...
    if args.query:
        URL = args.query[0]
        cursor.execute(sql_statements.select_db_entry(), (URL, ))
